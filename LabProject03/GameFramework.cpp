...
void CGameFramework::CreateSwapChain()						// O
{
	...
	::ZeroMemory(&dxgiSwapChainDesc, sizeof(DXGI_SWAP_CHAIN_DESC1));								// 메모리를 0으로 초기화 해줘

	dxgiSwapChainDesc.Width = m_nWndClientWidth;
	dxgiSwapChainDesc.Height = m_nWndClientHeight;
	/* Format : 픽셀의 포맷을 지정 UNORM = 정규화를 하겠다 ! */
	dxgiSwapChainDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	/* SampleDesc.Count : 다중 샘플링에서 1개의 픽셀을 평균값으로 표현하기 위한 픽셀의 수를 지정 4 = 2 x 2로 늘린다는 말 */
	dxgiSwapChainDesc.SampleDesc.Count = 1;							// 4배율을 지원하니 ? 
																	/* SampleDesc.Quality : 다중 샘플링 퀄리티를 지정 */
	dxgiSwapChainDesc.SampleDesc.Quality = 0;	// 최대로 표현할 수 있는 퀄리티를 넣어줘
												/* BufferUsage : 후면버퍼에 대한 표면 사용방식과 CPU의 접근 방법 */
	dxgiSwapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;						// 후면 버퍼를 렌더 타겟으로 사용할꺼야 !
	dxgiSwapChainDesc.BufferCount = m_nSwapChainBuffers;
	/* Scaling : 모니터 해상도에 맞게 확대하는 방법 */
	dxgiSwapChainDesc.Scaling = DXGI_SCALING_NONE;
	/* AlphaMode : 표면의 Alpha 값 투명도 , 동작을 식별 한다. */
	dxgiSwapChainDesc.AlphaMode = DXGI_ALPHA_MODE_UNSPECIFIED;
	dxgiSwapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
	/* Flages : 스왑 체인의 동작에 대한 선택 사항을 지정 ( 보통 0 )*/
	dxgiSwapChainDesc.Flags = 0;

	DXGI_SWAP_CHAIN_FULLSCREEN_DESC dxgiSwapChainFullScreenDesc;
	::ZeroMemory(&dxgiSwapChainFullScreenDesc, sizeof(DXGI_SWAP_CHAIN_FULLSCREEN_DESC));
	/* Numerator : 분모 , Denominator : 분자 => 1초에 60번 갱신할꺼야 ! */
	dxgiSwapChainFullScreenDesc.RefreshRate.Numerator = 60;
	dxgiSwapChainFullScreenDesc.RefreshRate.Denominator = 1;
	/* Scanline : 그래픽이 사용되는 화면에서 수평방향의 연속적인 화소의 모임 */
	/* ScanlineOrdering : 스캔라인 순서를 정한다. = Scanline을 어떤 순서대로 그릴거야? */
	dxgiSwapChainFullScreenDesc.ScanlineOrdering = DXGI_MODE_SCANLINE_ORDER_UNSPECIFIED;	// UNSPECIFIED : 드라이버 너가 알아서 해줘! 
																							/* Scaling : 전체 화면 전환시 어떻게 화면을 늘려줄래? */
	dxgiSwapChainFullScreenDesc.Scaling = DXGI_MODE_SCALING_UNSPECIFIED;					// UNSPECIFIED : 드라이버야 알아서 해줘 ㅎ
																							/* Windoed : 창모드인가? */
	dxgiSwapChainFullScreenDesc.Windowed = TRUE;											// 윈도우를 생성할 때 창모드로 생성할래

																							/* CreateSwapChainForHwnd( IUnKnown* pDevice , HWND hWnd , const DXGI_SWAP_CHAIN_DESC1* pDesc , const DXGI_SWAP_CHAIN_FULLSCREEN_DESC* pFullScreenDesc
																							IDXGIOutput* pRestrictToOutput , IDXGISwapChain1** ppSwapChain ) */
																							/* IUnKnown : COM 개체가 갖고 있는 인터페이스이다. 이 함수 기준으로 Direct12 에서는 CommandQueue 포인터를 사용한다. 절대 NULL 사용 불가능 */
																							/* HWND : 이 함수가 만들 스왑체인과 관련된 윈도우 핸들 = 화면 핸들. 절대 NULL 사용 불가 */
																							/* DXGI_SWAP_CHAIN_DESC1 : 스왑체인 서술자 = 스왑체인에 사용을 위해 설정한 옵션 (?) , 절대 NULL 사용 불가 */
																							/* DXGI_SWAP_CHAIN_FULLSCREEN_DESC : 전체화면 전환 스왑체인에 대한 서술자 = 전체화면 전환 스왑체인에 사용을 위해 설정한 옵션 , NULL 사용시 윈도우 화면 스왑체인 생성  */
																							/* (?)IDXGIOutput : 출력되는 내용을 IDXGIOutput 인터페이스( 모니터 ) 포인터에 제한하여 내보낼 것이다 . NULL일 경우 출력 대상을 제한하지 않겠다 라는 뜻 */
																							/* IDXGISwapChain1 : [Out] 이 함수에서 생성하는 스왑체인에 대한 포인터를 받는 변수에 대한 포인터 */
																							/* CreateSwapChainForHwnd : 스왑체인 결과를 출력할 윈도우에 대한 HWND 핸들과 연결된 스왑체인을 만들것이다. */
...
void CGameFramework::CreateDirect3DDevice()		 // O 		
{
	...
	/*          View Port를 클라이언트 영역 전체로 설정한다.          */

	m_d3dViewport.TopLeftX = 0;
	m_d3dViewport.TopLeftY = 0;
	/* static_cast : 기본 클래스에 대한 포인터에서 파생된 클래스에 대한 포인터로의 변환과 같은 연산에 사용 가능 */
	m_d3dViewport.Width = static_cast<float>(m_nWndClientWidth);
	m_d3dViewport.Height = static_cast<float>(m_nWndClientHeight);
	m_d3dViewport.MaxDepth = 1.0f;
	m_d3dViewport.MinDepth = 0.0f;

	/*          씨저 사각형을 주 윈도우의 클라이언트 전체 영역으로 설정          */
...
void CGameFramework::FrameAdvance()
{
	...
	/*     명령 할당자( Allocator ) , 명령 리스트를 리셋한다.     */

	/* CommandAllocator::Reset : CommandAllocator와 연관된 메모리를 다시 사용 한다. */
	HRESULT hResult = m_pd3dCommandAllocator->Reset();
	/* CommandList::Reset : 명령 리스트를 초기상태로 설정한다. */
	hResult = m_pd3dCommandList->Reset(m_pd3dCommandAllocator, NULL);

	/*     현재 렌더 타겟에 대한 프레젠트가 끝나길 기다린다.    */
	/*     프레젠트가 끝나면 렌더 타겟 버퍼의 상태는			*/
	/*     프레젠트 상태에서 렌더 타겟 상태로 변경된다.			*/

	/* 리소스 장벽 ( 리소스 사용의 전환 )을 서술한다. */
	D3D12_RESOURCE_BARRIER d3dResourceBarrier;
	::ZeroMemory(&d3dResourceBarrier, sizeof(D3D12_RESOURCE_BARRIER));
	/* D3D12_RESOURCE_BARRIER_TYPE_TRANSITION : 보조 리소스 집합의 전환을 설정한다. */
	/* 하위 리소스의 이전 및 이후 용도를 지정해 줘야 한다. */
	d3dResourceBarrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
	d3dResourceBarrier.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
	/* Render Target Buffer의 리소스를 지정 */
	d3dResourceBarrier.Transition.pResource = m_ppd3dRenderTargetBuffers[m_nSwapChainBufferIndex];
	/* D3D12_RESOURCE_STATE_PRESENT : 프레젠트 상태로 설정. */
	d3dResourceBarrier.Transition.StateBefore = D3D12_RESOURCE_STATE_PRESENT;
	/* D3D12_RESOURCE_STATE_RENDER_TARGET : 렌더 타겟 상태로 설정. */
	d3dResourceBarrier.Transition.StateAfter = D3D12_RESOURCE_STATE_RENDER_TARGET;
	d3dResourceBarrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
	m_pd3dCommandList->ResourceBarrier(1, &d3dResourceBarrier);

	/*     뷰포트와 씨저 사각형을 설정한다. 정확히는 레스터라이저 단계에 뷰포트 배열을 바인딩한다.     */
	
	m_pd3dCommandList->RSSetViewports(1, &m_d3dViewport);
	m_pd3dCommandList->RSSetScissorRects(1, &m_d3dScissorRect);

	/*     현재 렌더 타겟에 해당하는 서술자의 CPU주소(핸들)을 계산한다.     */

	D3D12_CPU_DESCRIPTOR_HANDLE d3dRtvCPUDescriptorHandle = m_pd3dRtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
	d3dRtvCPUDescriptorHandle.ptr += (m_nSwapChainBufferIndex * m_nRtvDescriptorIncrementSize);

	/*     렌더 타겟의 모든 픽셀을 한 가지 값으로 설정한다.     */
	...
	D3D12_CPU_DESCRIPTOR_HANDLE d3dDsvCPUDescriptorHandle = m_pd3dDsvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();	// 시작 주소를 Get!
	/* ClearDepthStencilView(D3D12_CPU_DESCRIPTOR_HANDLE DepthStencilView, D3D12_CLEAR_FLAGS ClearFlags, 
	FLOAT Depth, UINT8 Stencil, UINT NumRects, const D3D12_RECT* pRects */
	/* D3D12_CPU_DESCRIPTOR_HANDLE : 깊이-스텐실을 초기화 하기 위한 힘의 시작을 나타내는 CPU서술자 핸들 */
	/* D3D_CLEAR_FLAGS : 비트 OR연산의 조합으로 되어 있는 D3D12_CLEAR_FLAGS의 값을 나타낸다. 결과 값은 초기화 하고자 하는 값의 타입을 반환 */
	/* FLOAT : Depth Buffer의 초기 값 */
	/* UINT8 : Stencil Buffer 초기화 값 */
	/* D3D12_RECT : 초기화 할 Resource View의 사각형에 대한 D3D12_RECT 구조체. NULL사용시 전체 초기화 */
	m_pd3dCommandList->ClearDepthStencilView(d3dDsvCPUDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);

	/* 렌더 타겟과 깊이-스텐실 버퍼를 연결 */

	m_pd3dCommandList->OMSetRenderTargets(1, &d3dRtvCPUDescriptorHandle, TRUE, &d3dDsvCPUDescriptorHandle);

	/*     렌더링 코드      */

	/*     현재 렌더 타겟에 대한 렌더링이 끝나길 기다린다.     */
	/*     GPU가 렌더 타겟을 더이상 사용하지 않으면			*/
	/*     렌더타겟의 상태는 렌더타겟 상태 -> 프레젠트 상태     */
	...
	/* SwapChain을 프레젠트 한다. 프레젠트를 하면 현재 렌더 타겟 내용이 전면 버퍼로 옮겨지고 렌더 타겟 인덱스가 바뀐다. */

	DXGI_PRESENT_PARAMETERS dxgiPresentParameters;
	/* DXGI_PRESENT_PARAMETERS : 운영체제가 프레젠테이션을 최적화 하는데 도움이 되는 정보를 설명한다. */
	dxgiPresentParameters.DirtyRectsCount = 0;		// 0 : 전체 프레임을 업데이트 한다. 
	dxgiPresentParameters.pDirtyRects = NULL;
	dxgiPresentParameters.pScrollRect = NULL;
	dxgiPresentParameters.pScrollOffset = NULL;
	/* Presnet1( UINT SyncInterval, UINT PresentFlags, DXGI_PRESENT_PARAMETERS* pPresentParameters */
	/* UINT : 수직 프레임과 프레임의 표시를 동기화 하는 방법을 지정하는 정수. */
	/*		BitBlt 1 : 1번째 수직회귀 후에 프리젠트 , 플립 1 : 최소 1개의 수직회귀동안 프리젠트를 동기화 */
	/* UINT : 스왑 체인 프레젠트 옵션을 포함하는 정수 값 */
	/* DXGI_PRESENT_PARAMETERS : 표시 할 프레임의 업데이트 된 사각형 및 스크롤 정보를 설명 */
	/* 하지만 LabProject3에서는 Timer가 프레임 레이트를 관리해 주기 떄문에 사용을 안한다. */
...
{
	m_nFenceValue++;	// CPU 펜스의 값을 증가시킨다.
	...
	{
		// 펜스의 현재 값이 설정한 값보다 작으면 펜스의 현재 값이 설정한 값이 될 때 까지 기다린다.
		hResult = m_pd3dFence->SetEventOnCompletion(nFence, m_hFenceEvent);
		/* WaitForSingleObject : 이벤트 핸들을 할당 받을 때 까지 기다린다. CPU 할당 X */
